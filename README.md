...
Some tests are not run by default. For example, the Zypper tests are only run
on SUSE systems. They can be included by using the `-m` flag: `pytest -m zypper`.

[See TESTING.md for more details.](./TESTING.md)
